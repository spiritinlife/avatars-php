...
function getFinal($sP, $part, $partV, $theme) {
  ...
  $themes = [
    ...
    $regex = '/#(.*?);/';
    $result = preg_match_all($regex, $svgString, $matches, PREG_PATTERN_ORDER);
    $resultFinal = '';
    ...
    if ($result) {
      for ($i = 0; $i < count($matches[0]); $i++) {
        ...
        // echo 'replace' . ' ' .  $matches[0][$i] . ' ' . $colors[$i] . ' ' . PHP_EOL;
        // console.log('replace', result[i], colors[i])
        
        // replace first occurrence
        $pos = strpos($resultFinal, $matches[0][$i]);
        if ($pos !== false) {
            $resultFinal = substr_replace($resultFinal, $colors[$i].';', $pos, strlen($matches[0][$i]));
        }
      }
    }
...
function multiavatar ($string, $sansEnv = null, $ver = null) {
  $string .= '';
  ...
  $hash = '';
  if (mb_strlen($string) == 0) return $hash;

  $sha256Hash = hash('sha256', $string);

  $sha256Numbers = preg_replace('/\D/', "", $sha256Hash);
...
$p['top'] = round((47/100)*$p['top']) . '';
  ...
  foreach ($p as $part => $value ) {

    $nr = $p[$part];

    if ($nr > 31) {
      $nr = $nr - 32 . '';
      if (mb_strlen($nr) == 1) $nr = '0' . $nr;
      $p[$part] = $nr . 'C';
    }
    else if ($nr > 15) { 
      $nr = $nr - 16 . '';
      ...
      if (mb_strlen(($nr.'')) == 1) $p[$part] = '0' . $nr . 'A';
      else $p[$part] = $nr . 'A';
    }
  }
  ...
  foreach ($p as $part => $value ) {
    $partV = substr($p[$part],0,2);
    $theme = substr($p[$part],2,3);
    ...
    $final[$part] = getFinal($sP, $part, $partV, $theme);
